  ...
  <div class="diagnosis-container">
    ...
    <div class="card">
      ...
      <p class="monitor-description">
        连接到实时数据流，系统将持续接收电机运行数据并进行断条故障诊断。
      </p>
      ...
      <el-tabs type="border-card">
        ...
        <el-tab-pane label="特征指标">
          <div class="features-container">
            <el-row :gutter="50">
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('sideband_ratio') }">
                  <div class="feature-title">边带幅值比</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.sideband_ratio ? 
                      (latestDiagnosis.features.sideband_ratio * 100).toFixed(2) + '%' : '0.00%' }}
                  </div>
                  <div class="feature-desc">断条特征边带与基波幅值比</div>
                </div>
              </el-col>
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('normalized_fault_index') }">
                  <div class="feature-title">归一化故障指数</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.normalized_fault_index ? 
                      (latestDiagnosis.features.normalized_fault_index * 100).toFixed(2) + '%' : '0.00%' }}
                  </div>
                  <div class="feature-desc">断条故障的综合评估指数</div>
                </div>
              </el-col>
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('broken_bar_count') }">
                  <div class="feature-title">估计断条数</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.broken_bar_count ? 
                      latestDiagnosis.features.broken_bar_count : '0' }}
                  </div>
                  <div class="feature-desc">系统估计的断裂导条数量</div>
                </div>
              </el-col>
            </el-row>
          </div>
      ...
      <el-tabs>
        ...
        <el-tab-pane label="表格">
          <el-table :data="diagnosisHistoryTable" stripe style="width: 100%" height="500" border>
            ...
            <el-table-column prop="sidebandRatio" label="边带幅值比" width="150" />
            <el-table-column prop="normalizedFaultIndex" label="归一化故障指数" width="150" />
            <el-table-column prop="brokenBarCount" label="估计断条数" width="150" />
          </el-table>
        </el-tab-pane>
      </el-tabs>
    </div>
    ...
    <div v-if="connectionStatus === 'connected'" class="card">
      ...
      >
        ...
        <!-- 拆分特征指标为独立列 -->
        <el-table-column prop="features.sideband_ratio" label="边带幅值比" width="140">
          <template #default="scope">
            {{ (scope.row.features.sideband_ratio * 100).toFixed(2) + '%' }}
          </template>
        </el-table-column>
        <el-table-column prop="features.normalized_fault_index" label="归一化故障指数" width="140">
          <template #default="scope">
            {{ (scope.row.features.normalized_fault_index * 100).toFixed(2) + '%' }}
          </template>
        </el-table-column>
        <el-table-column prop="features.broken_bar_count" label="估计断条数" width="140">
          <template #default="scope">
            {{ scope.row.features.broken_bar_count }}
          </template>
        </el-table-column>
...
const diagnosisHistoryTable = computed(() => {
  return diagnosisHistory.value.map(item => ({
    ...
    score: item.score,
    status: item.status,
    sidebandRatio: item.features?.sideband_ratio ? (item.features.sideband_ratio * 100).toFixed(2) + '%' : '0.00%',
    normalizedFaultIndex: item.features?.normalized_fault_index ? (item.features.normalized_fault_index * 100).toFixed(2) + '%' : '0.00%',
    brokenBarCount: item.features?.broken_bar_count || 0
  }))
})
...
const connectWebSocket = () => {
  ...
  const saveHistoryToLocalStorage = () => {
    if (diagnosisHistory.value.length > 0) {
      try {
        ...
        localStorage.setItem('brokenBarDiagnosisHistory', JSON.stringify(historyToSave));
        localStorage.setItem('brokenBarHistoryTimestamp', Date.now().toString());
  ...
  const loadHistoryFromLocalStorage = () => {
    try {
      const savedHistory = localStorage.getItem('brokenBarDiagnosisHistory');
      const historyTimestamp = localStorage.getItem('brokenBarHistoryTimestamp');
      ...
      if (savedHistory && historyTimestamp) {
        ...
        if (age < 30 * 60 * 1000) {
          ...
          // 清除过期数据
          localStorage.removeItem('brokenBarDiagnosisHistory');
          localStorage.removeItem('brokenBarHistoryTimestamp');
        }
      }
    } catch (error) {
  ...
  wsConnection.value.onmessage = (event) => {
    try {
      ...
      // 检查是否为断条故障诊断结果
      if (data.fault_type === 'broken_bar') {
        // console.log('收到诊断结果:', data.status, data.score, data.features) // 优化：移除频繁日志
        ...
        if (data.time_series) {
          try {
            ...
            const values = data.time_series.values_a || []; // 修改为只获取values_a
        ...
        if (data.status === 'fault') {
          debouncedMessage('警告：检测到断条故障！', 'error', 5000);
        } else if (data.status === 'warning') {
          debouncedMessage('注意：检测到断条故障预警信号', 'warning', 3000);
        }
      }
    } catch (error) {
...
const isFeatureWarning = (featureName) => {
  ...
  if (featureName === 'sideband_ratio') return value > 0.05
  if (featureName === 'normalized_fault_index') return value > 0.1
  if (featureName === 'broken_bar_count') return value >= 1
...
const showFaultDetails = (fault) => {
  ElMessageBox.alert(
    `<div class="fault-details">
      ...
      <ul>
        <li>边带幅值比: ${(fault.features.sideband_ratio * 100).toFixed(2)}%</li>
        <li>归一化故障指数: ${(fault.features.normalized_fault_index * 100).toFixed(2)}%</li>
        <li>估计断条数: ${fault.features.broken_bar_count}</li>
      </ul>
...
const exportFaultRecord = (fault) => {
  ...
  const csvContent = [
    ...
    '',
    '特征指标,数值',
    `边带幅值比,${(fault.features.sideband_ratio * 100).toFixed(2)}%`,
    `归一化故障指数,${(fault.features.normalized_fault_index * 100).toFixed(2)}%`,
    `估计断条数,${fault.features.broken_bar_count}`,
    '',
    '诊断结论',
  ...
  link.setAttribute('download', `故障记录_断条_${fault.time.replace(/[: ]/g, '_')}.csv`)
...
const exportAllFaultRecords = () => {
  ...
  const rows = faultHistory.value.map(record => {
    ...
    const sidebandRatio = (record.features.sideband_ratio * 100).toFixed(2) + '%'
    const normalizedFaultIndex = (record.features.normalized_fault_index * 100).toFixed(2) + '%'
    const brokenBarCount = record.features.broken_bar_count
  ...
  const url = URL.createObjectURL(blob)
  link.setAttribute('href', url)
  link.setAttribute('download', `所有故障记录_断条_${new Date().toISOString().slice(0, 10)}.csv`)
  link.style.visibility = 'hidden'
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
