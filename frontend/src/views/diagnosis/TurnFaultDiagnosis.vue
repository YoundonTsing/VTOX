<template>
  <div class="diagnosis-container">
    <div class="page-header">
      <h2 class="page-title">匝间短路故障诊断</h2>
    ...
    <el-collapse-transition>
      <div v-if="showConfig">
        <threshold-config config-type="turn-fault" @update:thresholds="updateThresholds" />
      </div>
    </el-collapse-transition>
    ...
    <div class="card">
      ...
      <p class="monitor-description">
        连接到实时数据流，系统将持续接收电机运行数据并进行匝间短路故障诊断。
      </p>
      ...
      <el-tabs type="border-card">
        ...
        <el-tab-pane label="特征指标">
          <div class="features-container">
            <el-row :gutter="50">
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('current_imbalance') }">
                  <div class="feature-title">电流不平衡度</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.current_imbalance !== undefined && latestDiagnosis.features?.current_imbalance !== null ? 
                      (latestDiagnosis.features.current_imbalance * 100).toFixed(2) + '%' : 'N/A' }}
                  </div>
                  <div class="feature-desc">三相电流幅值偏差</div>
                </div>
              </el-col>
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('third_harmonic_ratio') }">
                  <div class="feature-title">三次谐波比例</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.third_harmonic_ratio !== undefined && latestDiagnosis.features?.third_harmonic_ratio !== null ? 
                      (latestDiagnosis.features.third_harmonic_ratio * 100).toFixed(2) + '%' : 'N/A' }}
                  </div>
                  <div class="feature-desc">三次谐波与基波幅值比</div>
                </div>
              </el-col>
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('negative_sequence_ratio') }">
                  <div class="feature-title">负序分量比例</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.negative_sequence_ratio !== undefined && latestDiagnosis.features?.negative_sequence_ratio !== null ? 
                      (latestDiagnosis.features.negative_sequence_ratio * 100).toFixed(2) + '%' : 'N/A' }}
                  </div>
                  <div class="feature-desc">负序电流分量占比</div>
                </div>
              </el-col>
            </el-row>
          </div>
      ...
      <el-tabs>
        ...
        <el-tab-pane label="表格">
          <el-table :data="diagnosisHistoryTable" stripe style="width: 100%" height="500" border>
            ...
            <el-table-column prop="currentImbalance" label="电流不平衡度" width="150" />
            <el-table-column prop="thirdHarmonicRatio" label="三次谐波比例" width="150" />
            <el-table-column prop="negativeSequenceRatio" label="负序分量比例" width="150" />
          </el-table>
        </el-tab-pane>
      </el-tabs>
    </div>
    ...
    <div v-if="connectionStatus === 'connected'" class="card">
      ...
      >
        ...
        <!-- 拆分特征指标为独立列 -->
        <el-table-column prop="features.current_imbalance" label="电流不平衡度" width="140">
          <template #default="scope">
            {{ scope.row.features.current_imbalance !== undefined && scope.row.features.current_imbalance !== null ? 
              (scope.row.features.current_imbalance * 100).toFixed(2) + '%' : 'N/A' }}
          </template>
        </el-table-column>
        <el-table-column prop="features.third_harmonic_ratio" label="三次谐波比例" width="140">
          <template #default="scope">
            {{ scope.row.features.third_harmonic_ratio !== undefined && scope.row.features.third_harmonic_ratio !== null ? 
              (scope.row.features.third_harmonic_ratio * 100).toFixed(2) + '%' : 'N/A' }}
          </template>
        </el-table-column>
        <el-table-column prop="features.negative_sequence_ratio" label="负序分量比例" width="140">
          <template #default="scope">
            {{ scope.row.features.negative_sequence_ratio !== undefined && scope.row.features.negative_sequence_ratio !== null ? 
              (scope.row.features.negative_sequence_ratio * 100).toFixed(2) + '%' : 'N/A' }}
          </template>
        </el-table-column>
...
const diagnosisHistoryTable = computed(() => {
  return diagnosisHistory.value.map(item => ({
    ...
    score: item.score,
    status: item.status,
    currentImbalance: item.features?.current_imbalance ? (item.features.current_imbalance * 100).toFixed(2) + '%' : '0.00%',
    thirdHarmonicRatio: item.features?.third_harmonic_ratio ? (item.features.third_harmonic_ratio * 100).toFixed(2) + '%' : '0.00%',
    negativeSequenceRatio: item.features?.negative_sequence_ratio ? (item.features.negative_sequence_ratio * 100).toFixed(2) + '%' : '0.00%'
  }))
})
    ...
    try {
      ...
      if (data.fault_type === 'turn_fault') {
        ...
        if (data.status === 'fault') {
          debouncedMessage('警告：检测到匝间短路故障！', 'error', 5000);
        } else if (data.status === 'warning') {
          debouncedMessage('注意：检测到匝间短路故障预警信号', 'warning', 3000);
        }
      }
    } catch (error) {
...
const saveHistoryToLocalStorage = () => {
  if (diagnosisHistory.value.length > 0) {
    try {
      ...
      localStorage.setItem('diagnosisHistory', JSON.stringify(historyToSave));
      localStorage.setItem('historyTimestamp', Date.now().toString());
    } catch (error) {
      console.error('[TurnFaultDiagnosis] 保存历史数据失败:', error);
    }
  }
}
...
const loadHistoryFromLocalStorage = () => {
  try {
    ...
    const historyTimestamp = localStorage.getItem('historyTimestamp');
    ...
    console.error('[TurnFaultDiagnosis] 加载历史数据失败:', error);
  }
  return false;
}
...
const isFeatureWarning = (featureName) => {
  ...
  if (featureName === 'current_imbalance') return value > 0.05
  if (featureName === 'third_harmonic_ratio') return value > 0.03
  if (featureName === 'negative_sequence_ratio') return value > 0.02
...
const showFaultDetails = (fault) => {
  ElMessageBox.alert(
    `<div class="fault-details">
      ...
      <ul>
        <li>电流不平衡度: ${(fault.features.current_imbalance * 100).toFixed(2)}%</li>
        <li>三次谐波比例: ${(fault.features.third_harmonic_ratio * 100).toFixed(2)}%</li>
        <li>负序分量比例: ${(fault.features.negative_sequence_ratio * 100).toFixed(2)}%</li>
      </ul>
...
const exportFaultRecord = (fault) => {
  ...
  const csvContent = [
    ...
    '',
    '特征指标,数值,阈值',
    `电流不平衡度,${(fault.features.current_imbalance * 100).toFixed(2)}%,5.00%`,
    `三次谐波比例,${(fault.features.third_harmonic_ratio * 100).toFixed(2)}%,3.00%`,
    `负序分量比例,${(fault.features.negative_sequence_ratio * 100).toFixed(2)}%,2.00%`,
    '',
    '诊断结论',
  ...
  link.setAttribute('download', `故障记录_${fault.time.replace(/[: ]/g, '_')}.csv`)
...
const exportAllFaultRecords = () => {
  ...
  const rows = faultHistory.value.map(record => {
    ...
    const currentImbalance = (record.features.current_imbalance * 100).toFixed(2) + '%'
    const thirdHarmonicRatio = (record.features.third_harmonic_ratio * 100).toFixed(2) + '%'
    const negativeSequenceRatio = (record.features.negative_sequence_ratio * 100).toFixed(2) + '%'
  ...
  const url = URL.createObjectURL(blob)
  link.setAttribute('href', url)
  link.setAttribute('download', `所有故障记录_${new Date().toISOString().slice(0, 10)}.csv`)
  link.style.visibility = 'hidden'
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
...
onMounted(() => {
  // 从localStorage加载阈值配置
  const savedThresholds = localStorage.getItem('turnFaultThresholds')
  if (savedThresholds) {
    try {
...
watch(() => props.autoStart, (newValue, oldValue) => {
