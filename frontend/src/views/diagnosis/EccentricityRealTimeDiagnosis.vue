  ...
  <div class="diagnosis-container">
    ...
    <div class="card">
      ...
      <p class="monitor-description">
        连接到实时数据流，系统将持续接收电机运行数据并进行偏心故障诊断。
      </p>
    ...
    <div v-if="connectionStatus === 'connected' && latestDiagnosis" class="card">
      <el-tabs type="border-card">
        ...
        <!-- 频谱分析 -->
        <el-tab-pane label="特征趋势分析">
          <div class="chart-container">
            <line-chart
              v-if="featureTrendData && featureTrendData.labels && featureTrendData.labels.length > 0"
              :chart-data="featureTrendData"
              chart-id="feature-trend-chart"
              :height="500"
              :options="featureTrendOptions"
              :key="`feature-trend-${Date.now()}`"
            />
            <div v-else class="no-data-message">
              <el-empty description="暂无特征趋势数据" />
            </div>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="特征指标">
          <div class="features-container">
            <el-row :gutter="50">
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('static_ecc_ratio') }">
                  <div class="feature-title">静态偏心率</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.static_ecc_ratio ? 
                      (latestDiagnosis.features.static_ecc_ratio * 100).toFixed(2) + '%' : '0.00%' }}
                  </div>
                  <div class="feature-desc">定子转子中心不重合度</div>
                </div>
              </el-col>
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('dynamic_ecc_ratio') }">
                  <div class="feature-title">动态偏心率</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.dynamic_ecc_ratio ? 
                      (latestDiagnosis.features.dynamic_ecc_ratio * 100).toFixed(2) + '%' : '0.00%' }}
                  </div>
                  <div class="feature-desc">转子旋转中心偏离几何中心程度</div>
                </div>
              </el-col>
              <el-col :span="8">
                <div class="feature-card" :class="{ 'feature-warning': isFeatureWarning('eccentricity_index') }">
                  <div class="feature-title">偏心故障综合指数</div>
                  <div class="feature-value">
                    {{ latestDiagnosis.features?.eccentricity_index ? 
                      (latestDiagnosis.features.eccentricity_index * 100).toFixed(2) + '%' : '0.00%' }}
                  </div>
                  <div class="feature-desc">反映偏心严重程度的综合指数</div>
                </div>
              </el-col>
            </el-row>
          </div>
      ...
      <el-tabs>
        ...
        <el-tab-pane label="表格">
          <el-table :data="diagnosisHistoryTable" stripe style="width: 100%" height="500" border>
            ...
            <el-table-column prop="staticEccRatio" label="静态偏心率(%)" width="150" />
            <el-table-column prop="dynamicEccRatio" label="动态偏心率(%)" width="150" />
            <el-table-column prop="eccentricityIndex" label="偏心故障综合指数(%)" width="150" />
          </el-table>
        </el-tab-pane>
      </el-tabs>
    </div>
    ...
    <div v-if="connectionStatus === 'connected'" class="card">
      ...
      >
        ...
        <!-- 拆分特征指标为独立列 -->
        <el-table-column prop="features.static_ecc_ratio" label="静态偏心率(%)" width="140">
          <template #default="scope">
            {{ (scope.row.features.static_ecc_ratio * 100).toFixed(2) + '%' }}
          </template>
        </el-table-column>
        <el-table-column prop="features.dynamic_ecc_ratio" label="动态偏心率(%)" width="140">
          <template #default="scope">
            {{ (scope.row.features.dynamic_ecc_ratio * 100).toFixed(2) + '%' }}
          </template>
        </el-table-column>
        <el-table-column prop="features.eccentricity_index" label="偏心故障综合指数(%)" width="140">
          <template #default="scope">
            {{ (scope.row.features.eccentricity_index * 100).toFixed(2) + '%' }}
          </template>
        </el-table-column>
...
// 加载阈值配置
const loadThresholdsFromLocalStorage = () => {
  // 从localStorage加载阈值配置
...
const featureTrendData = computed(() => {
  ...
  const staticEccRatio = sortedHistory.map(item => item.features?.static_ecc_ratio ? (item.features.static_ecc_ratio * 100).toFixed(2) : 0);
  const dynamicEccRatio = sortedHistory.map(item => item.features?.dynamic_ecc_ratio ? (item.features.dynamic_ecc_ratio * 100).toFixed(2) : 0);
  const eccentricityIndex = sortedHistory.map(item => item.features?.eccentricity_index ? (item.features.eccentricity_index * 100).toFixed(2) : 0);
...
const diagnosisHistoryTable = computed(() => {
  return diagnosisHistory.value.map(item => ({
    ...
    score: item.score,
    status: item.status,
    staticEccRatio: item.features?.static_ecc_ratio ? (item.features.static_ecc_ratio * 100).toFixed(2) : '0.00',
    dynamicEccRatio: item.features?.dynamic_ecc_ratio ? (item.features.dynamic_ecc_ratio * 100).toFixed(2) : '0.00',
    eccentricityIndex: item.features?.eccentricity_index ? (item.features.eccentricity_index * 100).toFixed(2) : '0.00'
  }))
})
...
const connectWebSocket = () => {
  ...
  const saveHistoryToLocalStorage = () => {
    if (diagnosisHistory.value.length > 0) {
      try {
        ...
        localStorage.setItem('eccentricityDiagnosisHistory', JSON.stringify(historyToSave));
        localStorage.setItem('eccentricityHistoryTimestamp', Date.now().toString());
  ...
  const loadHistoryFromLocalStorage = () => {
    try {
      const savedHistory = localStorage.getItem('eccentricityDiagnosisHistory');
      const historyTimestamp = localStorage.getItem('eccentricityHistoryTimestamp');
      ...
      if (savedHistory && historyTimestamp) {
        ...
        if (age < 30 * 60 * 1000) {
          ...
          // 清除过期数据
          localStorage.removeItem('eccentricityDiagnosisHistory');
          localStorage.removeItem('eccentricityHistoryTimestamp');
        }
      }
    } catch (error) {
  ...
  wsConnection.value.onmessage = (event) => {
    try {
      ...
      // 检查是否为偏心故障诊断结果
      if (data.fault_type === 'eccentricity') {
        console.log('收到偏心故障诊断结果:', data.status, data.score, data.features) // 恢复调试日志
        ...
        if (data.time_series) {
          try {
            ...
            const values_a = data.time_series.values_a || [];
            const values_b = data.time_series.values_b || [];
            const values_c = data.time_series.values_c || [];
            
            // 检查数据是否有效
            if (time.length > 0 && 
                values_a.length === time.length && 
                values_b.length === time.length && 
                values_c.length === time.length) {
        ...
        if (data.frequency_spectrum) {
          try {
            ...
            if (frequency.length > 0 && 
                amplitude_a.length === frequency.length && 
                amplitude_b.length === frequency.length && 
                amplitude_c.length === frequency.length) {
              
              // 限制数据点数量，避免图表过于密集
              const maxPoints = 50; // 优化：从100减少到50点，减少渲染负担
              const step = frequency.length > maxPoints ? Math.floor(frequency.length / maxPoints) : 1;
              
              const filteredFrequency = [];
              const filteredAmplitudeA = [];
              const filteredAmplitudeB = [];
              const filteredAmplitudeC = [];
              
              for (let i = 0; i < frequency.length; i += step) {
                filteredFrequency.push(frequency[i]);
                filteredAmplitudeA.push(amplitude_a[i]);
                filteredAmplitudeB.push(amplitude_b[i]);
                filteredAmplitudeC.push(amplitude_c[i]);
              }
        ...
        if (data.status === 'fault') {
          debouncedMessage('警告：检测到偏心故障！', 'error', 5000);
        } else if (data.status === 'warning') {
          debouncedMessage('注意：检测到偏心故障预警信号', 'warning', 3000);
        }
      }
    } catch (error) {
...
const isFeatureWarning = (featureName) => {
  ...
  if (featureName === 'static_ecc_ratio') return value > thresholds.warning // 静态偏心率
  if (featureName === 'dynamic_ecc_ratio') return value > thresholds.warning // 动态偏心率
  if (featureName === 'eccentricity_index') return value > thresholds.warning // 偏心综合指数
...
const showFaultDetails = (fault) => {
  ElMessageBox.alert(
    `<div class="fault-details">
      ...
      <ul>
        <li>静态偏心率: ${(fault.features.static_ecc_ratio * 100).toFixed(2)}%</li>
        <li>动态偏心率: ${(fault.features.dynamic_ecc_ratio * 100).toFixed(2)}%</li>
        <li>偏心故障综合指数: ${(fault.features.eccentricity_index * 100).toFixed(2)}%</li>
      </ul>
...
const exportFaultRecord = (fault) => {
  ...
  const csvContent = [
    ...
    '',
    '特征指标,数值',
    `静态偏心率,${(fault.features.static_ecc_ratio * 100).toFixed(2)}%`,
    `动态偏心率,${(fault.features.dynamic_ecc_ratio * 100).toFixed(2)}%`,
    `偏心故障综合指数,${(fault.features.eccentricity_index * 100).toFixed(2)}%`,
    '',
    '诊断结论',
  ...
  link.setAttribute('download', `故障记录_偏心故障_${fault.time.replace(/[: ]/g, '_')}.csv`)
...
const exportAllFaultRecords = () => {
  ...
  const rows = faultHistory.value.map(record => {
    ...
    const staticEccRatio = (record.features.static_ecc_ratio * 100).toFixed(2)
    const dynamicEccRatio = (record.features.dynamic_ecc_ratio * 100).toFixed(2)
    const eccentricityIndex = (record.features.eccentricity_index * 100).toFixed(2)
  ...
  const url = URL.createObjectURL(blob)
  link.setAttribute('href', url)
  link.setAttribute('download', `所有故障记录_偏心故障_${new Date().toISOString().slice(0, 10)}.csv`)
  link.style.visibility = 'hidden'
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
